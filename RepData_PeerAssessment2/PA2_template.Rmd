...
filename             <- "repdata-data-StormData"
filepath_destination <- paste("~/", filename, ".rds", sep = "")
download.file(url = filepath_source, destfile = filepath_destination, method = "curl")
data                 <- readRDS(filepath_destination)
...
```{r load_supplemental1}

setwd("~/")
filepath_source      <- "http://www.nber.org/census/pop/cencounts.csv"
filename             <- "cencounts"
filepath_destination <- paste("~/", filename, ".csv", sep = "")
download.file(url = filepath_source, destfile = filepath_destination)
# Only load information about year and total surplus/deficit
us_county_population <- read.csv(filepath_destination, stringsAsFactors = FALSE) 
...
```{r load_supplemental2, message=FALSE}

library(xlsx)
setwd("~/")
filepath_source      <- "http://www.whitehouse.gov/sites/default/files/omb/budget/fy2011/assets/hist01z1.xls"
filename             <- "hist01z1"
filepath_destination <- paste("~/", filename, ".xls", sep = "")
download.file(url = filepath_source, destfile = filepath_destination, method = "curl")
# Only load information about year and total surplus/deficit
us_federal_budget    <- read.xlsx(
  filepath_destination, 
  sheetIndex = 1, startRow = 4, colIndex = c(1,4), stringsAsFactors = FALSE) 
...
```
  ...
  * Subset needed years (1950-2011), convert values as stated in the document (values are in millions $ USD).
